...
export const ViewReport: React.FC<ViewReportProps> = ({ report, onBack }) => {
  ...
  const handlePrint = () => {
    ...
    const opened = openHrPrintPreview(
      ...
      `
        ...
        <p>${escapeHtml(report.id)} &middot; ${escapeHtml(report.period)}</p>
        <div class="summary">
          <div class="summary-card"><span>Status</span><strong>${escapeHtml(report.status)}</strong></div>
          <div class="summary-card"><span>Period</span><strong>${escapeHtml(report.period)}</strong></div>
          <div class="summary-card"><span>Downloads</span><strong>${escapeHtml(report.downloads)}</strong></div>
          <div class="summary-card"><span>File Size</span><strong>${escapeHtml(report.fileSize)}</strong></div>
        </div>
        <h2>Report Details</h2>
        <div class="details">
          <div class="detail-row"><span>Report Title</span><strong>${escapeHtml(report.title)}</strong></div>
          <div class="detail-row"><span>Category</span><strong>${escapeHtml(report.category)}</strong></div>
          <div class="detail-row"><span>Reporting Period</span><strong>${escapeHtml(report.period)}</strong></div>
          <div class="detail-row"><span>Generated By</span><strong>${escapeHtml(report.generatedBy)}</strong></div>
          <div class="detail-row"><span>Date Generated</span><strong>${escapeHtml(formattedDate)}</strong></div>
          <div class="detail-row"><span>File Size</span><strong>${escapeHtml(report.fileSize)}</strong></div>
        </div>
      `
    );
  ...
  return (
    <>
      <div className="hr-report-print-area flex flex-col gap-5">
      {/* Back */}
      <button
        onClick={onBack}
        className="no-print flex items-center gap-1.5 text-sm text-gray-500 hover:text-primary transition-colors w-fit"
      >
        <ArrowLeft size={16} />
        Back to Reports
      </button>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="no-print flex items-center gap-2">
            ...
            {report.status === "Generated" && (
              <>
                <Button variant="outline" className="gap-2" onClick={handlePrint} disabled={printing} loading={printing}>
